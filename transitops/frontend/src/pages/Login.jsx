      ...
      <div className="flex-1 flex items-center justify-center px-6 bg-canvas">
        <div className="w-full max-w-[380px]">
          ...
          <p className="text-[13px] text-ink-500 mt-1 mb-6">
            {isRegister ? 'Register an office account to get access to the dispatch dashboard.' : 'Access your dispatch dashboard.'}
          </p>
            ...
            {isRegister && (
              <Field label="Role">
                <select
                  ...
                  className={inputClass}
                >
                  {Object.entries(ROLE_LABELS).filter(([value]) => value !== 'driver').map(([value, label]) => (
          ...
          <div className="mt-6 pt-5 border-t border-line text-center">
            <p className="text-[13px] text-ink-500">
              Are you a driver?{' '}
              <Link to="/signup" className="text-pine font-medium hover:text-pine-dark">
                Apply for a driver account
              </Link>
            </p>
          </div>
        </div>
